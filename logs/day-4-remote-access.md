...
- Ubuntu Server lost static IP after router changes — got x.x.x.x
  from pfSense LAN DHCP instead of x.x.x.x
- Fixed by configuring static IP in /etc/netplan/00-installer-config.yaml
- Applied netplan configuration to permanently set x.x.x.x
...
| Proxmox | https://x.x.x.x:8006 | https://proxmox.mondol.dev |
...
| Pi-hole | http://x.x.x.x:8053/admin | - |
| Kali Linux | x.x.x.x:3389 (RDP) | - |
| Minecraft Bedrock | x.x.x.x:19132 | - |
...
- **Problem:** After reboots and network changes Ubuntu Server received
  x.x.x.x from pfSense LAN DHCP instead of x.x.x.x
