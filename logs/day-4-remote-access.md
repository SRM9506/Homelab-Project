...
- Tested multiple ports (51820, 51821, 443) — all blocked
...
- Ubuntu Server lost static IP after router changes — got 192.168.2.101
  from pfSense LAN DHCP instead of 192.168.1.156
- Fixed by configuring static IP in /etc/netplan/00-installer-config.yaml
- Applied netplan configuration to permanently set 192.168.1.156
...
- Clickable checkboxes with localStorage progress tracking
...
| Proxmox | https://192.168.1.200:8006 | https://proxmox.mondol.dev |
...
| Pi-hole | http://192.168.1.156:8053/admin | - |
| Kali Linux | 192.168.1.150:3389 (RDP) | - |
| Minecraft Bedrock | 192.168.1.156:19132 | - |
...
- **Diagnosis:** Verizon Fios blocks all inbound UDP traffic at ISP
  level regardless of router configuration. Verified with portchecker.co
  showing all ports closed including 51820, 443, and 8006
...
- **Problem:** After reboots and network changes Ubuntu Server received
  192.168.2.101 from pfSense LAN DHCP instead of 192.168.1.156
- **Diagnosis:** pfSense LAN DHCP server was active on the same bridge
  as the main network, competing with router DHCP
- **Resolution:** Configured permanent static IP via netplan in
  /etc/netplan/00-installer-config.yaml with dhcp4: no
