...
- Removed passphrase from the key for seamless passwordless auth
...
manually pasted public key into authorized_keys. Also removed passphrase
...
- VS Code Remote SSH spins up its own SSH connection separate from the
  terminal — passwordless auth (SSH keys, no passphrase) is required for
  it to work reliably
