...
- Configured WAN interface (em0) at 192.168.1.160
- Added LAN interface (em1) at 192.168.2.1 with DHCP range 192.168.2.100-200
- Added permanent WAN firewall rule to allow HTTPS GUI access
- Configured DNS Resolver with host overrides for all services
- Added Access List allowing 192.168.1.0/24 to query DNS
- Disabled HTTP_REFERER enforcement for proxy access
- Set router DNS to 192.168.1.160 so all devices use pfSense DNS
...
- Accessible at http://192.168.1.156:8053/admin
...
- World data stored at /home/shark742/minecraft
...
| Kali Linux RDP | 192.168.1.150:3389 | Security Lab |
...
- **Resolution:** Switched all host overrides from .local to .lan.
  Added Access List in DNS Resolver allowing 192.168.1.0/24.
...
- **Problem:** Kali received 192.168.2.x IP from pfSense LAN DHCP
  instead of 192.168.1.x from router
- **Resolution:** Set static IP 192.168.1.150 directly in
